...
def modex_select(texts, adjacency='text', tau=0.8, goodness_of_cut='conductance'):
    """
    ...
    if adjacency == 'semantics':
        A = compute_semantics_adjacency_matrix(agent_names, agent_responses)
    elif adjacency == 'text':
        A = compute_text_adjacency_matrix(agent_names, agent_responses)
    elif adjacency == 'both':
        A_sem = compute_semantics_adjacency_matrix(agent_names, agent_responses)
...
def compute_semantics_adjacency_matrix(agent_names, agent_responses):
    emb_encoder = SentenceTransformer('all-MiniLM-L6-v2').to('cuda')   # có thể đổi model khác nếu muốn
...
def main(args):
    ...
    for i, gen in enumerate(tqdm(generations, desc="Processing generations")):
        ...
        selected_index = modex_select(cleaned_texts, adjacency='semantics', tau=0.45)
